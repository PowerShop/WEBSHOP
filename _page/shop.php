...
<html lang="en">
  <head>
    <title>SHOP : <?php @ini_set("display_errors", "0");  echo $_GET['name']; ?></title>
    <!-- Required meta tags -->
    <meta charset="utf-8">
  ...
  <body class="col-md-8 mx-auto font mt-2">
      ...
      $category = $_GET['name'];
            $query = "SELECT * FROM `item` WHERE `server` = '$server' AND `category` = '$category'";
                $query2 = "SELECT * FROM `user` WHERE `username` = '$username'";
                 ...
                 <div class="card center col-md-6 sm-auto mx-auto">
...
if(isset($_GET['itemname'])){
    ...
    swal({
        title: 'Are You Sure?',
        text: 'คุณต้องการที่จะซื้อสินค้า $name ใราคา $price พอยท์ หรือไม่?',
        icon: 'warning',
